# Privacy Policy URL Configuration

**Status:** ✅ COMPLETED  
**Date:** 2025-01-27

---

## Privacy Policy URL

**Production URL:**
```
https://meloshemo.github.io
```

**Hosting:** GitHub Pages  
**Last Updated:** November 04, 2025

---

## Store Submission Configuration

### Google Play Store

**Location:** Store listing → Privacy policy

**URL to use:**
```
https://meloshemo.github.io
```

**Steps:**
1. Go to Google Play Console
2. Navigate to Store listing
3. Find "Privacy policy" field
4. Enter: `https://meloshemo.github.io`
5. Save and publish

---

### Apple App Store

**Location:** App Information → Privacy Policy URL

**URL to use:**
```
https://meloshemo.github.io
```

**Steps:**
1. Go to App Store Connect
2. Select your app
3. Go to App Information
4. Find "Privacy Policy URL" field
5. Enter: `https://meloshemo.github.io`
6. Save

---

## Verification

✅ Privacy Policy is accessible at: https://meloshemo.github.io  
✅ Last updated: November 04, 2025  
✅ Contact email: HemoAI@hotmail.com  
✅ GDPR/KVKK compliant content

---

## Notes

- Privacy Policy is hosted on GitHub Pages (free hosting)
- URL is stable and permanent
- No additional configuration needed
- Update the HTML file in the repository to update the policy

---

**Last Updated:** 2025-01-27

