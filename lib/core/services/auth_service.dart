...
class AuthService {
  ...
  Future<AuthResult> login(String email, String password) async {
    try {
      ...
      if (userData == null) {
        ...
        return AuthResult.failure(LocalizationService().getString('user_not_found'));
      }

      // Verify password
      if (userData.passwordHash != hashedPassword) {
        _logger.w('Invalid password for user: $email');
        return AuthResult.failure(LocalizationService().getString('password_incorrect'));
      }

      // Check if user is active
      if (!userData.isActive) {
        _logger.w('Inactive user attempted login: $email');
        return AuthResult.failure(LocalizationService().getString('account_is_deactivated'));
      }
      ...
      await prefs.setUserInfo(
        ...
        user.phone,
        emailVerified: true,
        phoneVerified: user.phone.isNotEmpty,
      );
      ...
      return AuthResult.failure('${loc.getString('login_failed')}: ${e.toString()}');
    }
  }
  ...
  Future<AuthResult> register(UserModel user, String password) async {
    try {
      ...
      if (existingUser != null) {
        ...
        return AuthResult.failure(LocalizationService().getString('user_already_exists'));
      }
      ...
      final userId =
          await _database.into(_database.users).insert(userCompanion);
      ...
      await prefs.setUserInfo(
        ...
        createdUser.phone,
        emailVerified: true,
        phoneVerified: createdUser.phone.isNotEmpty,
      );
      ...
      final loc = LocalizationService();
      return AuthResult.failure('${loc.getString('registration_failed')}: ${e.toString()}');
    }
  }
  ...
  Future<bool> changePassword(
    ...
    try {
      ...
      _logger.i(LocalizationService().getString('password_changed_successfully'));
      return true;
    } catch (e, stackTrace) {
  ...
  Future<bool> resetPassword(String email) async {
    try {
      ...
      await prefs.saveCustomSettings(
          'reset_token_expiry_${userData.id}', tokenExpiry.toIso8601String());
      ...
      _logger.i(
          'Password reset email ${emailSent ? "sent" : "failed"} for: $email');
      return true;
    } catch (e, stackTrace) {
  ...
  Future<bool> resetPasswordWithToken(
    ...
    try {
      ...
      final prefs = await PreferencesService.getInstance();
      final storedToken =
          prefs.getCustomSetting<String>('reset_token_${userData.id}');
      final expiryStr =
          prefs.getCustomSetting<String>('reset_token_expiry_${userData.id}');
  ...
  Future<void> logout() async {
    try {
      _logger.i(LocalizationService().getString('logged_out'));
  ...
  Future<void> _attemptCloudRestore(int userId) async {
    try {
      ...
      final password =
          await SecureStoreService().read('backup_password_for_sync');

      if (password == null || password.isEmpty) {
        // No password stored, skip restore
        _logger.i(LocalizationService().getString('auto_restore_no_backup'));
        return;
      }
  ...
  Future<void> _logAuditEvent(
      int userId, String action, Map<String, dynamic> details) async {
    try {
