...
class DataImportService {
  ...
  Future<ImportResult> importFromEDevlet({
    ...
    try {
      ...
      if (kDebugMode) {
        ...
        print('🏛️ Attempting e-Devlet import for TC: ${tcKimlik.substring(0, 3)}***');
      }
  ...
  Future<ImportResult> importFromFile() async {
    try {
      ...
      if (kDebugMode) {
        print('📄 Importing from file: ${file.name} ($extension)');
      }
  ...
  Future<ImportResult> _parsePDFFile(PlatformFile file) async {
    // PDF parsing with OCR would go here
    // This would integrate with Google ML Kit Text Recognition
    return ImportResult.error(
